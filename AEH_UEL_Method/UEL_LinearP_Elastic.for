C-----------------------------------------------------------------
C     UEL for asymptotic homogenization method in the case of Linear perturbation step
C-----------------------------------------------------------------     
...
C-----------------------------------------------------------------   
     ...
     1 ONE=1.D0, TWO=2.D0)  
      DIMENSION TMATRX(6), EMATRX(6),TE(6), TM(6) ! TE Thermal Expansion
      ...
      DIMENSION D(6,6), S(6,NDOFEL), CMATRX(6)    ! Elastic Matrix D and S = D*B
      DIMENSION SSTRAIN(6), SSTRESS(6), BMATRX(6) ! Stress and Strain
...
C-----------------------------------------------------------------
        DO K1=1,NDOFEL
          DO K2=1,6
            RHS(K1,1)=RHS(K1,1)+BT(K1,K2)*D(K2,KSTEP)*WEGT*BH         
          ENDDO
C          
          DO K2=1,NDOFEL
            DO KK=1,6
...
C      		
      ENDDO  
C-----------------------------------------------------------------	
C     Test if it's the second call                   
C-----------------------------------------------------------------       
      IF (LFLAGS(3).EQ.100) THEN
        DO I=1,NDOFEL
          DO K=1,NDOFEL
            RHS(I,1)=RHS(I,1)-AMATRX(I,K)*DU(K,1)
          ENDDO
        ENDDO   
C-----------------------------------------------------------------	
C     Calculate strain and elastic matrix of each element          
C-----------------------------------------------------------------        
        DO K1=1,6
          SSTRAIN(K1)=ZERO
          EMATRX(K1)=ZERO 
...
C			  
        DO K1=1,6
          DO K2=1,NDOFEL
            SSTRAIN(K1)=SSTRAIN(K1)+B(K1,K2)*DU(K2,1)
          ENDDO
        ENDDO
C	    
        DO K1=1,6
	    ...
	    ENDDO
	    EMATRX(K1)=(D(K1,KSTEP)-TMATRX(K1))*WEGT*BH/VOLUME
        ENDDO
C     
        WRITE(6,*) EMATRX
C       
