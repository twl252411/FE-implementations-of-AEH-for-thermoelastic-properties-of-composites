...
# ----------------------------- Modify Job name ------------------------------
#
lines[1] = '** Job name: Job-p3 Model name: Model-1\n'
...
#
staid1 = lines.index('*End Instance\n')
lines.insert(staid1+2,'*Node\n')
lines.insert(staid1+3,'      1,   '+str(xlenp*1.1)+',           0.,           0.\n')
lines.insert(staid1+4,'*Node\n')
lines.insert(staid1+5,'      2,           0.,   '+str(ylenp*1.1)+',           0.\n')
lines.insert(staid1+6,'*Node\n')
lines.insert(staid1+7,'      3,           0.,           0.,   '+str(zlenp*1.1)+'\n')
...
#-----------------------------  ------------------------------
#
addlns1 = addlnsS1 + addlnsS2 + addlnsE3 + addlnsE4 + addlnsE5 + addlnsE6 + addlnsE7 + addlnsE8
...
#----------------------------- Combine the file ------------------------------
#
staid2 = lines.index('*Nset, nset=Set-'+str(ndnum)+', instance=Part-1-1\n') + 2
...
addlns3.append('** \n')
addlns3.append('** Name: BC-19 Type: Displacement/Rotation\n')
addlns3.append('*Boundary\n')
addlns3.append('Set-V-8, 1, 1\n')
addlns3.append('Set-V-8, 2, 2\n')
addlns3.append('Set-V-8, 3, 3\n')

#----------------------------- Input Reaction Force -----------------------------
#
RF1 = np.loadtxt(f'{new_job_name}_ini_nd_for_1.txt', delimiter=',')
RF2 = np.loadtxt(f'{new_job_name}_ini_nd_for_2.txt', delimiter=',')
RF3 = np.loadtxt(f'{new_job_name}_ini_nd_for_3.txt', delimiter=',')
RF4 = np.loadtxt(f'{new_job_name}_ini_nd_for_4.txt', delimiter=',')
RF5 = np.loadtxt(f'{new_job_name}_ini_nd_for_5.txt', delimiter=',')
RF6 = np.loadtxt(f'{new_job_name}_ini_nd_for_6.txt', delimiter=',')
#
#----------------------------- PBCs and Loadings -----------------------------
#
addlns4 = []
...
for istep in range(6):
	...
	addlns4.append('** ----------------------------------------------------------------\n')
	addlns4.append('** \n')
	addlns4.append(f'** STEP: Step-{istep+1}\n')
	addlns4.append('** \n')
	addlns4.append(f'*Step, name=Step-{istep+1}, nlgeom=NO, perturbation\n')
	addlns4.append('*Static\n')
	addlns4.append('** \n')
	addlns4.append('** BOUNDARY CONDITIONS\n')
	addlns4.append('** \n')
	addlns4.append(f'** Name: BC-{istep*3+1} Type: Displacement/Rotation\n')
	addlns4.append('*Boundary\n')
	addlns4.append('Set-RP-1, 1, 1\n')
	addlns4.append('Set-RP-1, 2, 2\n')
	addlns4.append('Set-RP-1, 3, 3\n')
	addlns4.append(f'** Name: BC-{istep*3+2} Type: Displacement/Rotation\n')
	addlns4.append('*Boundary\n')
	addlns4.append('Set-RP-2, 1, 1\n')
	addlns4.append('Set-RP-2, 2, 2\n')
	addlns4.append('Set-RP-2, 3, 3\n')
	addlns4.append(f'** Name: BC-{istep*3+3} Type: Displacement/Rotation\n')
	addlns4.append('*Boundary\n')
	...
	#
	for inum in range(ndnum):
		addlns4.append(f'** Name: Load-{istep+1}-{inum+1} Type: Concentrated force\n')
		addlns4.append('*Cload, op=NEW\n')
		if istep == 0:
			[xx, yy, zz] = RF1[inum,1:4]
		elif istep == 1:
			[xx, yy, zz] = RF2[inum,1:4]
		elif istep == 2:
			[xx, yy, zz] = RF3[inum,1:4]
		elif istep == 3:
			[xx, yy, zz] = RF4[inum,1:4]
		elif istep == 4:
			[xx, yy, zz] = RF5[inum,1:4]
		else:
			[xx, yy, zz] = RF6[inum,1:4]
		#
		addlns4.append(f'Set-{inum+1}, 1, {xx}\n')
		addlns4.append(f'Set-{inum+1}, 2, {yy}\n')
		addlns4.append(f'Set-{inum+1}, 3, {zz}\n')
	#
	addlns4.append('** \n')
...
#----------------------------- Combine the file ------------------------------
#
staid3 = lines.index('** STEP: Step-1\n') - 2
lines = lines[0:staid3] + addlns3 + addlns4
#
#----------------------------- Save the file ------------------------------
#
with open(f'{new_job_name}_2.inp', mode='w') as f:
	f.writelines(lines)