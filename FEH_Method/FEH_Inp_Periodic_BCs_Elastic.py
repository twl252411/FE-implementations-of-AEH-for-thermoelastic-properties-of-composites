...
#----------------------------- Parameters ------------------------------
#
# satin_num = 5  # 2 for plain and 3 for twill
# if satin_num == 4:
#     width, thickness, extra_t = 4.0, 4.0, 0.04
#     height = width
#     gama = np.arctan(np.sqrt(2.0)) * 180 / np.pi
#     cae_name = f'4D_braided_rve.cae'
#     job_name = f'Job_4D_braided_FEH_elastic'
#     phase_num = 5
# else:
#     alpha = 45.0
#     sb, r = 1.0, 2.0
...
#     height = thickness
#     cae_name = f'5D_braided_rve.cae'
#     job_name = f'Job_5D_braided_FEH_elastic'
#     phase_num = 6
...
phase_num = 3
#
# rve_size = np.array([1.0, 1.0, 1.0]) * 100.2
xlenp, ylenp, zlenp = rve_size / 2.
xlenm, ylenm, zlenm = - rve_size / 2.
pbc_disp = rve_size

for ivol, imsh, irve in product([10], [55], [1]):
	#job_name = f'Job_1d_elastic_{ivol}_{irve}'
	#----------------------------- Open the inp file ------------------------------
	#
	...
	#----------------------------- Modify Job name ------------------------------
	#
	lines[1] = '** Job name: Job-pbc Model name: Model-1\n'
	...
	for ind in NdRS:
		...
		jnd = _find_match(
			NdLS, (2, 3), ls_map, ls_order, (ndcord[ind-1,2], ndcord[ind-1,3])
		)
		if jnd is not None:
	...
	for ind in NdTS:
		...
		jnd = _find_match(
			NdDS, (1, 3), ds_map, ds_order, (ndcord[ind-1,1], ndcord[ind-1,3])
		)
		if jnd is not None:
	...
	for ind in NdFS:
		...
		jnd = _find_match(
			NdBS, (2, 1), bs_map, bs_order, (ndcord[ind-1,2], ndcord[ind-1,1])
		)
		if jnd is not None:
	...
	addlns3.append('** \n')
	addlns3.append('** Name: BC-0 Type: Displacement/Rotation\n')
	...
	addlns4 = []
	steps = [
		{"name": "Step-1", "bc": [(1, pbc_disp[0], None, None), (2, None, None, None), (3, None, None, None)]},
		{"name": "Step-2", "bc": [(1, None, None, None), (2, None, pbc_disp[1], None), (3, None, None, None)]},
		{"name": "Step-3", "bc": [(1, None, None, None), (2, None, None, None), (3, None, None, pbc_disp[2])]},
		{"name": "Step-4", "bc": [(1, None, pbc_disp[0] / 2., None), (2, pbc_disp[1] / 2., None, None), (3, None, None, None)]},
		{"name": "Step-5", "bc": [(1, None, None, pbc_disp[0] / 2.), (2, None, None, None), (3, pbc_disp[2] / 2., None, None)]},
		{"name": "Step-6", "bc": [(1, None, None, None), (2, None, None, pbc_disp[1] / 2.), (3, None, pbc_disp[2] / 2., None)]},
	]

	for step_idx, step in enumerate(steps, start=1):
		addlns4.append("** ----------------------------------------------------------------\n")
		addlns4.append("** \n")
		addlns4.append(f"** STEP: {step['name']}\n")
		addlns4.append("** \n")
		addlns4.append(f"*Step, name={step['name']}, nlgeom=NO, perturbation\n")
		addlns4.append("*Static\n")
		addlns4.append("** \n")
		addlns4.append("** BOUNDARY CONDITIONS\n")
		addlns4.append("** \n")

		for rp_idx, (rp_num, disp_x, disp_y, disp_z) in enumerate(step["bc"], start=1):
			addlns4.append(f"** Name: BC-{(step_idx - 1) * 3 + rp_idx} Type: Displacement/Rotation\n")
			addlns4.append("*Boundary\n")
			if disp_x is not None:
				addlns4.append(f"Set-RP-{rp_num}, 1, 1, {disp_x}\n")
			else:
				addlns4.append(f"Set-RP-{rp_num}, 1, 1\n")
			if disp_y is not None:
				addlns4.append(f"Set-RP-{rp_num}, 2, 2, {disp_y}\n")
			else:
				addlns4.append(f"Set-RP-{rp_num}, 2, 2\n")
			if disp_z is not None:
				addlns4.append(f"Set-RP-{rp_num}, 3, 3, {disp_z}\n")
			else:
				addlns4.append(f"Set-RP-{rp_num}, 3, 3\n")
