...
savefile = f'Woven{satin_num}_Homogenized_Stiffness_FEH.txt'
#
...
for iStep in range(6):
    ...
    stress = np.array([v.data for v in F1V], dtype=float)
    ivol = np.array([v.data for v in F2V], dtype=float)
    homo_stiff[iStep, :] += np.sum(stress * ivol[:, None], axis=0) / VRVE
odb.close()
#
